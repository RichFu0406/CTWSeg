...
def main():
    ...
    args.names_path = project_path(args.names_path)
    result_root = project_path(args.result_root)
    checkpoint_folder, args.save_folder = create_labeled_result_dirs(
        result_root,
        args.labeled_point,
    )
    if args.model_path:
    ...
    os.environ["CUDA_VISIBLE_DEVICES"] = str(args.test_gpu)
    logger = get_logger()
    logger.info(args)
    ...
    if os.path.isfile(args.model_path):
        ...
        state_dict = checkpoint.get('teacher_state_dict', checkpoint['state_dict'])
...
def data_prepare():
    ...
    area_prefix = 'Area_{}_'.format(args.test_area)
    data_list = [
        item[:-4]
        for item in data_list
        if item.startswith(area_prefix) and item.endswith('.npy')
    ]
    if not data_list:
        raise RuntimeError(
            "No evaluation scenes found for Area_{} in '{}'".format(
                args.test_area,
                args.data_root,
            )
        )
